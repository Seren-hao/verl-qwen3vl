...
os.environ["RAY_USE_MULTIPROCESSING"] = "1"
os.environ["RAY_DISABLE_FORK"] = "1"
...
class TaskRunner:
    ...
    def __init__(self):
        import torch
        import torch_npu
        print(f"1.--------------------------------------------> is_available : {torch.npu.is_available()}")
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            from recipe.dance_grpo_mm.diffusion_workers import DiffusionActorRolloutWorker
    ...
    def run(self, config):
        print(f"2.--------------------------------------------> is_available : {torch.npu.is_available()}")
        ...
        tokenizer = Tokenizer(args.mm.data.dataset_param.tokenizer_config).get_tokenizer()
        cfg = args.mm.data.dataset_param.preprocess_parameters.cfg
        text_processer = TextProcesser(
            tokenizer=tokenizer,
            enable_text_preprocessing=args.mm.data.dataset_param.enable_text_preprocessing,
            text_preprocess_methods=args.mm.data.dataset_param.text_preprocess_methods,
            use_clean_caption=False,
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, text_processer, is_train=True, max_samples: int = -1):
